...
randnum = 1000         #Number of networks with edge deletion
numthr = 0.1           #Fraction of original edges to be deleted
...
outfilename = '..//output//'+species+'_bowtierandom'+str(int(numthr*100))+'deledges.tsv'
...
for i in range(randnum):     #For each network with random edge deletion
    netn = net.copy()
    ecount = 0
    while (1>0):
        gene12 = np.random.choice(edges,1)[0]
        [gene1,gene2] = gene12.split("\t")
        if netn.has_edge(gene1,gene2):
            netn.remove_edge(gene1,gene2)
            ecount += 1
            if ecount >= ethr:
                break
